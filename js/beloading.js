...
var beloading = function beload (options, callback) {
  ...
  options = {
    ...
    manually: options.manually || 'false' // manually remove the loading page
  }
  ...
  beloadingReturn.__init__ = function __init__ () {
    ...
    if (options.trail === 'false' && options.manually === 'false') {
      if (document.readyState === "complete") beloadingReturn.stop()
      else $(window).on('load', beloadingReturn.stop)
    }
  }
